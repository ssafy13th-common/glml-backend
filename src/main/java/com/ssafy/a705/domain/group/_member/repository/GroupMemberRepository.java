...
public interface GroupMemberRepository extends JpaRepository<Participant, Long> {

    // 특정 그룹에 속한 멤버들
    @Query(value = "SELECT gm FROM Participant gm "
    ...
    @Query("SELECT new com.ssafy.a705.domain.group._member.dto.response.GroupMemberProfileRes(gm.group.id, m.profileUrl) "
            + "FROM Participant gm JOIN gm.member m "
    ...
    @Query("SELECT gm FROM Participant gm "
            + "WHERE gm.id IN :groupMemberIds "
            + "AND gm.deletedAt IS NULL")
    List<Participant> findGroupMembersByGroupMemberIds(
    ...
    @Query("SELECT gm FROM Participant gm " +
            ...
            "AND m.deletedAt IS NULL " +
            "AND g.deletedAt IS NULL")
    Optional<Participant> findGroupMemberByMemberIdAndGroupId(@Param("memberId") Long memberId,
            @Param("groupId") Long groupId);
    ...
    @Query("SELECT gm FROM Participant gm " +
            ...
            "AND m.deletedAt IS NULL " +
            "AND g.deletedAt IS NULL")
    Optional<Participant> findGroupMemberByMemberEmailAndGroupId(@Param("email") String email,
            @Param("groupId") Long groupId);

    // 그룹에 속한 멤버들의 email 집합 - 일괄적으로 받아서 한 번의 쿼리로 중복체크를 하기 위함
    @Query("SELECT gm.member.id FROM Participant gm "
            + "WHERE gm.group.id = :groupId "
